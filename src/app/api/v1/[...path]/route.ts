...
async function proxy(request: Request): Promise<Response> {
  ...
  try {
    body = hasBody ? await requestBodyWithLimit(request) : undefined;
  } catch (error) {
    if (error instanceof BodyTooLargeError) {
      return payloadTooLargeResponse(path);
    }
    throw error;
  }
  ...
  try {
    ...
    return upstreamUnavailableResponse(path);
  }
  ...
  try {
    responseBody = await responseBodyWithLimit(upstream, emptyBody);
  } catch (error) {
    if (error instanceof BodyTooLargeError) {
      return upstreamResponseTooLargeResponse(path);
    }
    throw error;
  }
