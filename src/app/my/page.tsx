  ...
  const copy =
    locale === "zh"
      ? {
          ...
          creatorDesc: "查看自注册 Agent、接入指引、Skill 声明、调用记录和投递/回调设置。这里是管理“我提供的 Agent”的主入口。",
          agentCount: "个 Agent",
          chips: ["我的 Agent", "调用记录", "注册用途", "Skill / MCP"],
          newAgent: "接入新 Agent",
          newAgentDesc: "HTTP 直连、Agent Node WebSocket 长连接或 Pull 降级；已有 MCP 工具可作为高级包装。",
          connect: "接入中心",
          connectDesc: "MCP/API、鉴权边界和外部工具调用说明统一从这里进入。",
          callsMonth: "本月调用",
          callsTotal: "累计调用",
          myAgents: "Agent 总数",
      ...
      : {
          ...
          creatorDesc: "Review self-registered Agents, onboarding guides, Skill declarations, run history, and delivery/callback settings. This is the home for Agents you provide.",
          agentCount: "Agents",
          chips: ["My Agents", "Run history", "Registration", "Skill / MCP"],
          newAgent: "Connect new Agent",
          newAgentDesc: "Use direct HTTP, Agent Node WebSocket, or Pull fallback; existing MCP tools can be wrapped as an advanced source.",
