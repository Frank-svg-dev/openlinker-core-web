  ...
  const copy =
    locale === "zh"
      ? {
          hub: "创作者中心",
          current: "调用记录",
          kicker: "创作者 / Agent 调用记录",
          heading: `${"AGENT"} · 被调用记录`,
          lead: "这里展示这个 Agent 被用户、访问令牌或 MCP 工具触发的运行记录。自注册 Agent 会绑定到生成注册用途访问令牌的 creator 用户，因此也会出现在同一份列表里。",
          onboarding: "接入配置",
          back: "返回创作者中心",
          listTitle: "被调用记录",
      ...
      : {
          hub: "Creator Hub",
          current: "Run Records",
          kicker: "Creator / Agent runs",
          heading: `${"AGENT"} · Run records`,
          lead: "This shows runs triggered by users, access tokens, or MCP tools. Self-registered Agents are bound to the creator user that generated their registration token, so those runs appear here too.",
