...
const WORKFLOW_COPY = {
  zh: {
    ...
    compareStatus: (id: string, changed: number) => `已对比 ${id.slice(0, 8)} 与当前运行，差异节点 ${changed} 个。`,
    pauseStatus: "workflow_run 已暂停，worker 不会继续认领。",
    resumeStatus: "workflow_run 已恢复到队列。",
    cancelStatus: "workflow_run 已取消。",
    retryStatus: "已复制失败运行的输入并重新入队。",
    agentNotCallable: "这个 Agent 当前不可运行，无法加入工作流。",
    productionKicker: "生产工作流",
    productionTitle: "现在会保存工作流，并真实调用每个 Agent 节点",
    productionLead: "Builder 现在可以编辑节点依赖顺序；系统会自动检查流程顺序，并并行运行互不依赖的节点。运行后可按步骤重跑并对比两次运行。",
    tasksLink: "去任务闭环",
    a2aLink: "查看 A2A",
    agentLibraryKicker: "Agent 节点库",
    agentLibraryTitle: "真实 Agent 节点库",
    emptyAgents: "暂时没有可调用公开 Agent。先去市场确认 Agent 状态，或接入一个 Agent Node / WebSocket Agent。",
    added: "已在画布中",
    ...
    viewChildRun: "查看子运行",
    rerunning: "重跑中",
    rerunFromStep: "从此步骤重跑",
    recentSaved: "最近保存",
    itemCount: (count: number) => `${count} 个`,
    needSignInShort: "需登录",
    signInSave: "登录后查看并保存工作流",
    noSaved: "还没有保存记录。点击“保存并运行”会创建第一条。",
    savedNodes: (count: number) => `${count} 个 Agent 节点 · 查看历史`,
    runHistory: "运行历史",
    runCount: (count: number) => `${count} 条`,
    runAttemptSummary: (attempt: number, max: number, steps: number) =>
      `第 ${attempt}/${max} 次执行 · ${steps} 个步骤`,
    chooseWorkflow: "选择工作流",
    historyLogin: "登录后可查看工作流运行历史。",
    readingHistory: "正在读取历史...",
    noHistory: "选择一个已保存工作流，或运行一次新工作流后查看历史。",
    comparing: "对比中",
    compareCurrentRun: "与当前运行对比",
  ...
  en: {
    ...
    savedDescription: "This workflow was saved and run from the Builder.",
    ...
    agentNotCallable: "This Agent is not currently runnable, so it cannot be added to the workflow.",
    productionKicker: "Production workflow",
    productionTitle: "Workflows are saved and each Agent node runs for real",
    productionLead: "The builder supports editing node dependencies. The system validates flow order and runs independent nodes in parallel. After running, you can rerun step by step and compare two runs.",
    tasksLink: "Task loop",
    a2aLink: "View A2A",
    agentLibraryKicker: "Agent library",
    agentLibraryTitle: "Real Agent library",
    emptyAgents: "No callable public Agents yet. Check Agent status in the market, or connect an Agent Node / WebSocket Agent.",
    ...
    runAttemptSummary: (attempt: number, max: number, steps: number) =>
      `Attempt ${attempt}/${max} · ${steps} steps`,
...
export function WorkflowBuilder({
  ...
  const loadWorkflowRuns = async (workflowID: string) => {
    ...
    try {
      ...
      setError(localizedErrorMessage(err, locale, copy.historyReadFailed));
      setWorkflowRuns([]);
  ...
  const rerunStep = async (run: WorkflowRunResponse, nodeKeyValue: string) => {
    ...
    try {
      ...
      setError(localizedErrorMessage(err, locale, copy.stepRerunFailed));
    } finally {
      setRerunBusy(null);
    }
  };
  ...
  const compareWithCurrent = async (run: WorkflowRunResponse) => {
    ...
    try {
      ...
      setError(localizedErrorMessage(err, locale, copy.compareFailed));
    } finally {
      setControlBusy(null);
    }
  };
  ...
  const controlWorkflowRun = async (
    ...
    try {
      ...
      setError(localizedErrorMessage(err, locale, copy.controlFailed));
    } finally {
      setControlBusy(null);
    }
  };
  ...
  const runWorkflow = async () => {
    ...
    try {
      ...
      const failureReason = localizedErrorMessage(
        new Error(String(latest.error_message ?? latest.last_worker_error ?? "")),
        locale,
        "",
      );
      setStatus(
      ...
      setError(localizedErrorMessage(err, locale, copy.runFailedFallback));
    ...
    <div className="space-y-8">
      <div className="ol-panel ol-panel-pad border-[color:var(--ol-green)]/25 bg-[color:var(--ol-green-soft)]">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="ol-kicker">{copy.productionKicker}</div>
      ...
      <section className="grid gap-6 xl:grid-cols-[290px_minmax(0,1fr)_340px]">
        <aside className="ol-panel ol-panel-pad h-fit">
          <div className="ol-kicker">{copy.agentLibraryKicker}</div>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="mt-2 flex items-center justify-between text-[11px] font-black uppercase text-[color:var(--ol-subtle)]">
                      <span>{availabilityStatusLabel(agent.availability?.status ?? "unknown", locale, agent.availability?.label)}</span>
        ...
        <section className="ol-panel overflow-hidden">
          ...
          <div className="overflow-x-auto p-5">
            <div className="relative min-h-[430px] min-w-[760px] rounded-[18px] border border-dashed border-[color:var(--ol-line)] bg-[linear-gradient(90deg,rgba(49,118,237,0.05)_1px,transparent_1px),linear-gradient(180deg,rgba(23,133,123,0.06)_1px,transparent_1px)] bg-[size:42px_42px]">
              <svg
                ...
                return (
                  ...
                  >
                    ...
                    <span className="mt-3 flex items-center justify-between text-[11px] font-black uppercase tracking-[0.05em] text-[color:var(--ol-subtle)]">
                      {node.role}
                      {isFailed ? (
                        <span className="text-red-600">{runStatusLabel("failed", locale)}</span>
                      ) : isDone ? (
                        <span className="text-[color:var(--ol-green)]">{runStatusLabel("success", locale)}</span>
                      ) : isActive ? (
                        <span className="text-[color:var(--ol-primary-dark)]">
                          {runStatusLabel("running", locale)}
                        </span>
                      ) : null}
                    </span>
                  </button>
          ...
          <div className="ol-panel ol-panel-pad">
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="shrink-0 text-[11px] font-black uppercase text-[color:var(--ol-subtle)]">
                        {step?.status ? runStatusLabel(step.status, locale) : running ? runStatusLabel("running", locale) : copy.queued}
                      </span>
                    </div>
                    {step?.run_id ? (
                      <Link
            ...
            {createdWorkflow || runResult ? (
              <div className="mt-4 rounded-[14px] bg-[color:var(--ol-soft)] p-3 text-[12px] leading-relaxed text-[color:var(--ol-muted)]">
                {createdWorkflow ? <div>{copy.workflowID}: {createdWorkflow.id}</div> : null}
                {runResult ? <div>{copy.workflowRunID}: {runResult.id}</div> : null}
              </div>
            ) : null}
            {runResult ? (
              <RunActions
              ...
              {!isAuthenticated ? (
                ...
                workflowRuns.map((run) => (
                  ...
                  >
                    ...
                    <div className="mt-1 text-[11px] font-bold text-[color:var(--ol-subtle)]">
                      {copy.runAttemptSummary(run.attempt_count ?? 0, run.max_attempts ?? 3, run.steps?.length ?? 0)}
                    </div>
  ...
  const comparisonCopy =
    locale === "zh"
      ? {
          changed: "已变化",
          same: "相同",
          subRunChanged: "子运行已变化",
          outputChanged: "输出已变化",
        }
      : {
          changed: "Changed",
          same: "Same",
          subRunChanged: "Child run changed",
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="font-black uppercase">
                {step.changed ? comparisonCopy.changed : comparisonCopy.same}
              </span>
            </div>
            <div className="mt-1 text-[11px] font-bold">
              {step.base_status ? runStatusLabel(step.base_status, locale) : "-"} →{" "}
              {step.candidate_status ? runStatusLabel(step.candidate_status, locale) : "-"}
              {step.run_changed ? ` · ${comparisonCopy.subRunChanged}` : ""}
              {step.output_changed ? ` · ${comparisonCopy.outputChanged}` : ""}
            </div>
          </div>
        ))}
      </div>
