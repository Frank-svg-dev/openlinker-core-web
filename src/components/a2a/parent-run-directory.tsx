  ...
  const copy =
    locale === "zh"
      ? {
          ...
          body: "每条会话从最初运行开始，汇总其中所有并行、连续和多层 Agent 调用。",
          searchLabel: "搜索 Agent、Skill 或运行 ID",
          searchPlaceholder: "输入 Agent 名称、slug、Skill、标签或运行 ID",
          searchAction: "搜索",
          clearSearch: "清除",
      ...
      : {
          title: "My Collaboration Sessions",
          total: (n: number) => `${n} total`,
          body: "Each session starts with the first run and groups every parallel, sequential, and nested Agent call beneath it.",
          searchLabel: "Search Agents, Skills, or run IDs",
          searchPlaceholder: "Agent name, slug, Skill, tag, or run ID",
          searchAction: "Search",
          clearSearch: "Clear",
          loadFailed: "Unable to load collaboration sessions.",
          empty: "No collaboration sessions yet. They will appear after you run an Agent that delegates work.",
          searchEmpty: (q: string) => `No collaboration sessions matched "${q}".`,
          viewAgents: "View My Agents",
          viewA2A: "View A2A Setup",
          parentAgent: "Starting Agent",
