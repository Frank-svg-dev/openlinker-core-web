...
const TYPE_HINTS: Record<"webhook" | "slack", { label: Record<Locale, string>; placeholder: string; hint: Record<Locale, string> }> = {
  webhook: {
    ...
    hint: {
      zh: "运行完成后平台向该 URL POST 投递内容，并在 X-OpenLinker-Signature 中带 HMAC-SHA256 签名。",
  ...
  const copy =
    locale === "zh"
      ? {
          ...
          webhookSecret: "用于校验平台投递时的 HMAC-SHA256 签名。",
          slackSecret: "Slack incoming webhook URL 本身即是 secret，已存储；下方仅记录平台生成的备用 secret。",
          warning: "警告：",
