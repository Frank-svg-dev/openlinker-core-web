  ...
  const copy =
    locale === "zh"
      ? {
          ...
          secretOnce: "Agent Token 明文仅显示一次。推荐直接复制接入启动包给 Agent；完整注册、Skill 映射和运行闭环由 /skill/publish-agent 维护。",
          ...
          tokenBoundary: "这不是设置页里的 MCP/API User Token，只能用于",
          tokenBoundarySuffix: "注册并运行 Agent。详细接入步骤、Skill 映射规则和 Agent Node runtime_ws / Pull 降级要求统一维护在",
          generated: (n: number) => `已生成 ${n} 个 Agent Token 记录。它们只用于 Agent 接入和运行，不用于 MCP/API 调用，也不会出现在设置页的 User Token 列表里。`,
          approvals: "待处理审批",
      ...
      : {
          ...
          secretOnce: "The Agent Token plaintext is shown only once. Copy the startup packet to the Agent; full registration, Skill mapping, and runtime loop rules live in /skill/publish-agent.",
          ...
          tokenBoundary: "This is not the MCP/API User Token from Settings. It can only call",
          tokenBoundarySuffix: "to register and run an Agent. Detailed onboarding steps, Skill mapping, and Agent Node runtime_ws / Pull fallback rules live in",
          generated: (n: number) => `${n} Agent Token records generated. They are only for Agent onboarding and runtime, not MCP/API calls, and do not appear in Settings User Tokens.`,
